...
function buildModuleGraph(sourceFiles) {
  ...
  for (const modulePath of sourceFiles) {
    ...
    const dependencies = parseDependencySpecifiers(sourceCode).map((specifier) => resolveDependencyPath(modulePath, specifier)).filter(Boolean);
...
function ensureActionableError(error, modulePath) {
  ...
  return new Error(`Falha ao processar m\xF3dulo ${modulePath}: ${reason}`);
}
...
async function buildIncrementalArtifacts({ moduleGraph, srcDir, distDir, cacheFilePath, parallelism }) {
  ...
  const profile = (await Promise.all(
    chunks.map(
      (chunk) => processModuleChunk({ chunk, moduleGraph, srcDir, distDir, previousCache, nextCacheModules: nextCache.modules })
    )
  )).flat().sort((a, b) => a.module.localeCompare(b.module));
...
async function runBuild(cwd = process.cwd(), options = {}) {
  ...
  if (performanceBudget.status === "fail") {
    throw new Error(`Build bloqueado por regress\xE3o cr\xEDtica de performance: ${performanceBudget.violations.join(" ")}`);
  }
  return {
    ...performanceBudget,
    incrementalBuild
